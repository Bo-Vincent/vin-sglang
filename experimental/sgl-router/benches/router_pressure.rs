...
use sgl_router::load_monitor::proto::load_monitor_service_client::LoadMonitorServiceClient;
use sgl_router::load_monitor::proto::{
    LoadReport, RankLoad, ReportStatus, Worker as ReportWorker, WorkerType,
};
#[cfg(router_pressure_legacy_snapshot)]
use sgl_router::load_monitor::LoadMonitorSnapshot;
#[cfg(not(router_pressure_legacy_snapshot))]
use sgl_router::load_monitor::SchedulingSnapshot;
use sgl_router::load_monitor::{bind_and_serve, LoadMonitor};
...
#[cfg(router_pressure_legacy_snapshot)]
type PolicySnapshot = LoadMonitorSnapshot;
#[cfg(not(router_pressure_legacy_snapshot))]
type PolicySnapshot = SchedulingSnapshot;

#[cfg(router_pressure_legacy_snapshot)]
fn capture_policy_snapshot(monitor: &LoadMonitor) -> PolicySnapshot {
    monitor.snapshot()
}

#[cfg(not(router_pressure_legacy_snapshot))]
fn capture_policy_snapshot(monitor: &LoadMonitor) -> PolicySnapshot {
    monitor.scheduling_snapshot()
}

#[cfg(router_pressure_legacy_snapshot)]
fn snapshot_len(snapshot: &PolicySnapshot) -> usize {
    snapshot.workers.len()
}

#[cfg(not(router_pressure_legacy_snapshot))]
fn snapshot_len(snapshot: &PolicySnapshot) -> usize {
    snapshot.fresh_loads.len()
}
...
fn build_fixture(endpoint_count: usize, ranks: usize, mode: WorkerMode) -> Fixture {
    tokio::runtime::Runtime::new()
        ...
        .block_on(async move {
            ...
            let (monitor, server) = bind_and_serve(config)
                .await
                .expect("bind benchmark load monitor");
            ...
            let mut client =
                LoadMonitorServiceClient::connect(format!("http://{}", server.local_addr()))
                    .await
                    .expect("connect benchmark load reporter");
            for index in 0..endpoint_count {
                client
                    .report(tokio_stream::iter(vec![
                        report(index, mode, ranks, 1),
                        report(index, mode, ranks, 2),
                    ]))
                    .await
                    .expect("seed benchmark load report");
            }
            monitor.stop_registrations().await;
            server.shutdown(&monitor).await;
...
fn prefill_decision(
    ...
    #[cfg(not(router_pressure_legacy_snapshot))]
    let ctx = ctx.with_candidate_membership(range.membership());
    let proposal = policy
...
fn decode_decision(domain: &CandidateDomain, snapshot: &PolicySnapshot) -> WorkerId {
