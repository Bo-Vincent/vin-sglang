...
[2026-08-05-cache-candidate-routing.md](2026-08-05-cache-candidate-routing.md) 为准。
分支：`vin/router-v2`
...
1. **先写失败测试**：候选域内 P2 pair、primary/backup admission、当前域 fallback；
   Cache-Aware 后续改为 Top-K candidate Admission/tournament，不再使用 Cache Benefit
   backup gate。
...
4. 为 Session-Aware 实现 `session_affinity_mode = bucket | global-rebind | global-preserve`。Cache-
   Aware 后续固定为全局 Indexer candidate path，不复用该 Session mode。
