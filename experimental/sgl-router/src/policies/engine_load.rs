                ...
                A: SeqAccess<'de>,
            {
                ...
                let native_cache = match seq.next_element::<u64>()? {
                    ...
                    Some(num_waiting_uncached_tokens) => {
                        let num_total_tokens = seq.next_element()?.ok_or_else(|| {
                            de::Error::missing_field("num_total_tokens")
                        })?;
                        let max_running_requests = seq.next_element()?.ok_or_else(|| {
                            de::Error::missing_field("max_running_requests")
                        })?;
                        let total_prefill_uncached_tokens = seq.next_element()?.ok_or_else(|| {
                            de::Error::missing_field("total_prefill_uncached_tokens")
                        })?;
                        let total_prefill_busy_us = seq.next_element()?.ok_or_else(|| {
                            de::Error::missing_field("total_prefill_busy_us")
                        })?;
            ...
            .filter_map(|url| {
                ...
                for rank in required {
                    ...
                    max_total_num_tokens = max_total_num_tokens
                        .saturating_add(load.max_total_num_tokens);
