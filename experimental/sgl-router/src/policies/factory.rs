...
fn build_kind(
    ...
    Ok(match kind {
        ...
        PolicyKind::ScorePolicy => {
            build_score_policy(model, &tree, &tokenizers, &block_size_oracle, engine_load)?
        }
        PolicyKind::PrefixCache => {
...
fn build_score_composition(
    ...
    for t in spec {
        ...
        let inner = build_kind(t.kind, &m, tree, tokenizers, oracle, engine_load)?;
