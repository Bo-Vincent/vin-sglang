...
mod tests {
    ...
    #[test]
    fn affinity_policies_accept_a_queue_pressure_threshold() {
        for policy_args in [
            "--policy session_aware --pressure-abs-threshold-ms 75",
            "--policy cache_aware --kv-indexer-endpoint http://indexer:50051 --pressure-abs-threshold-ms 75",
        ] {
            let config = cfg_of(policy_args).expect("queue threshold should be accepted");
            let affinity = config.model.affinity.expect("affinity config");
    ...
    fn cache_aware_can_disable_its_pressure_guard() {
        let config = cfg_of(
            "--policy cache_aware --kv-indexer-endpoint http://indexer:50051 --disable-pressure-guard",
        )
    ...
    #[test]
    fn queue_pressure_threshold_rejects_invalid_values_and_unrelated_policies() {
        for argument in [
