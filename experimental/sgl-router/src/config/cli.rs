...
pub struct Cli {
    ...
    // ---- shared Session-Aware / Cache-Aware tuning ----
    ...
    /// Session/Cache affinity primary 在两个候选都通过 hard admission 后，是否
    /// 可以按软策略逃逸到 backup。
    #[arg(long, value_enum)]
    ...
    /// 关闭 Cache-Aware 可选的 Cache Benefit gate。
    #[arg(long)]
    pub disable_cache_benefit: bool,
...
impl Cli {
    ...
    pub fn into_config(self) -> Result<Config> {
        ...
        let tuned_affinity = self.session_id_header.is_some()
            ...
            || self.disable_cache_benefit;
        if tuned_affinity && !affinity_policy {
            return Err(anyhow!(
                "affinity tuning flags require --policy session_aware or cache_aware"
            ));
        }
        ...
        if self.disable_cache_benefit && self.policy != PolicyKind::CacheAware {
            return Err(anyhow!(
                "--disable-cache-benefit requires --policy cache_aware"
            ));
        }
        ...
        let affinity = if affinity_policy {
            ...
            Some(AffinityConfig {
                ...
                pressure_rel_threshold,
                cache_benefit: !self.disable_cache_benefit,
            })
        } else {
            None
        };
...
mod tests {
    ...
    fn cache_aware_accepts_indexer_endpoint_and_rejects_affinity_knobs_elsewhere() {
        let config = cfg_of(
            "--policy cache_aware --kv-indexer-endpoint http://indexer:50051 --disable-cache-benefit",
        )
        .unwrap();
    ...
    fn rejects_affinity_options_that_cannot_affect_the_selected_policy() {
        ...
        let session_cache_benefit = cfg_of("--policy session_aware --disable-cache-benefit")
            .expect_err("Cache Benefit has no meaning for Session-Aware")
            .to_string();
        assert!(
