...
NATIVE_CACHE_AUDIT_FIELDS = (
    ...
    "fallback_power_of_two_decisions",
    "fallback_power_of_two_proposals",
    "fallback_monitor_decisions",
    "fallback_router_local_decisions",
    "fallback_zero_snapshot_decisions",
)
SHORTEST_TTFT_AUDIT_FIELDS = (
...
ZMQ_POLICY_AUDIT_FIELDS = (
    "lookup_decisions",
    "cache_holder_selections",
    "threshold_fallbacks",
    "load_imbalance_fallbacks",
    "block_size_fallbacks",
)
...
def validate_summary(case: Mapping[str, object], summary: Mapping[str, object]) -> None:
    ...
    if policy == "cache_aware":
        ...
        if (
            audit["cache_candidate_decisions"] + audit["fallback_power_of_two_decisions"]
            != request_count
        ):
            raise RuntimeError("native Cache-Aware audit does not cover every request")
        if audit["fallback_power_of_two_proposals"] < audit["fallback_power_of_two_decisions"]:
            raise RuntimeError("native Cache-Aware fallback has no P2 proposal")
        if audit["fallback_monitor_decisions"] != audit["fallback_power_of_two_proposals"]:
            raise RuntimeError("native Cache-Aware fallback does not use LoadMonitor")
        if audit["actual_cache_metrics"] <= 0:
            raise RuntimeError("native Cache-Aware case has no cache metric evidence")
        if (
            audit["router_local_decisions"] != 0
            or audit["zero_snapshot_decisions"] != 0
            or audit["fallback_router_local_decisions"] != 0
        ...
        for field in SHORTEST_TTFT_AUDIT_FIELDS:
            if not isinstance(audit.get(field), int):
        ...
        if audit["router_local_decisions"] != 0 or audit["zero_snapshot_decisions"] != 0:
            raise RuntimeError("Shortest-TTFT case fell back from LoadMonitor")
    elif policy == "power_of_two":
        audit = summary.get("power_of_two_audit")
        if not isinstance(audit, Mapping):
            raise RuntimeError("power-of-two case has no audit")
        for field in POWER_OF_TWO_AUDIT_FIELDS:
            if not isinstance(audit.get(field), int):
                raise RuntimeError(f"power-of-two audit has no integer {field}")
        if audit["power_of_two_decisions"] != request_count:
            raise RuntimeError("power-of-two audit does not cover every request")
        if (
            audit["monitor_decisions"] != request_count
            or audit["router_local_decisions"] != 0
            or audit["zero_snapshot_decisions"] != 0
        ):
            raise RuntimeError("power-of-two audit did not use a fresh LoadMonitor snapshot")
    elif policy == "cache_aware_zmq":
        audit = summary.get("zmq_policy_audit")
        if not isinstance(audit, Mapping):
            raise RuntimeError("ZMQ case has no audit")
        for field in ZMQ_POLICY_AUDIT_FIELDS:
            if not isinstance(audit.get(field), int):
                raise RuntimeError(f"ZMQ audit has no integer {field}")
        direct_paths = (
            audit["lookup_decisions"]
            + audit["load_imbalance_fallbacks"]
            + audit["block_size_fallbacks"]
        )
        if direct_paths != request_count:
            raise RuntimeError("ZMQ audit does not cover every request")
        if audit["cache_holder_selections"] + audit["threshold_fallbacks"] != audit[
            "lookup_decisions"
        ]:
            raise RuntimeError("ZMQ audit does not cover every lookup outcome")
        if audit["cache_holder_selections"] <= 0:
            raise RuntimeError("ZMQ case has no cache-holder selection")
...
def analyze_results(
    ...
    for (endpoint_count, policy, workload), primary_samples in sorted(primary.items()):
        ...
        native_cache_audit_totals: dict[str, int] = defaultdict(int)
        shortest_ttft_audit_totals: dict[str, int] = defaultdict(int)
        power_of_two_audit_totals: dict[str, int] = defaultdict(int)
        zmq_policy_audit_totals: dict[str, int] = defaultdict(int)
        ...
        for _, summary in samples:
            ...
            if isinstance(audit, Mapping):
                for field in NATIVE_CACHE_AUDIT_FIELDS:
                    native_cache_audit_totals[field] += int(audit[field])
            audit = summary.get("shortest_ttft_audit")
            if isinstance(audit, Mapping):
                for field in SHORTEST_TTFT_AUDIT_FIELDS:
                    shortest_ttft_audit_totals[field] += int(audit[field])
            audit = summary.get("power_of_two_audit")
            if isinstance(audit, Mapping):
                for field in POWER_OF_TWO_AUDIT_FIELDS:
                    power_of_two_audit_totals[field] += int(audit[field])
            audit = summary.get("zmq_policy_audit")
            if isinstance(audit, Mapping):
                for field in ZMQ_POLICY_AUDIT_FIELDS:
                    zmq_policy_audit_totals[field] += int(audit[field])
        group = {
        ...
        if native_cache_audit_totals:
            group["native_cache_audit"] = dict(native_cache_audit_totals)
        if shortest_ttft_audit_totals:
            group["shortest_ttft_audit"] = dict(shortest_ttft_audit_totals)
        if power_of_two_audit_totals:
            group["power_of_two_audit"] = dict(power_of_two_audit_totals)
        if zmq_policy_audit_totals:
            group["zmq_policy_audit"] = dict(zmq_policy_audit_totals)
        groups.append(group)
