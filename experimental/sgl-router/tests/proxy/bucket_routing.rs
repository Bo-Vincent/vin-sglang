...
use sgl_router::config::{
    ActiveLoadConfig, AffinityAwareRange, AffinityConfig, BucketConfig, BucketSpec, BucketStage,
    Config, DiscoveryBackend, ModelConfig, ObservabilityConfig, PolicyKind, ProxyConfig,
    ServerConfig, SloBucketPolicy, StaticUrlsDiscoveryConfig,
};
...
fn build_ctx(
    ...
    Arc::new(AppContext::new(
        config, tokenizers, proxy, registry, policies,
    ))
}
...
#[tokio::test]
async fn global_first_session_affinity_can_keep_a_cross_length_bucket_primary() {
    ...
    short_bucket.max_input_tokens = Some(256);
    ...
    long_bucket.min_input_tokens = Some(257);
    ...
    let ctx = build_ctx(
        ...
        Some(AffinityConfig {
            aware_range: AffinityAwareRange::GlobalFirst,
            ..Default::default()
        }),
    );
